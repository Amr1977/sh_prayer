...
now_ts=$(date +%s)
is_dst=$(date +%Z | grep -qE 'EDT|CEST|DST' && echo 1 || echo 0)
...
for prayer in fajr dhuhr asr maghrib isha; do
  ...
  prayer_time_24=$(date -j -f "%I:%M %p" "$adjusted_time" +"%H:%M")
  full_time_local="$(date +%F) $prayer_time_24"
  prayer_ts=$(date -j -f "%F %H:%M" "$full_time_local" +%s 2>/dev/null)
...
while true; do
  ...
  if [ "$remaining" -le 0 ]; then
    say -v "$VOICE" "It is time for $next_prayer prayer."
